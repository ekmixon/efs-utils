...
def _get_mock_config(dns_name_format='{az}.{fs_id}.efs.{region}.{dns_name_suffix}', dns_name_suffix='amazonaws.com',
                     config_section='mount', has_fallback_to_mount_target_ip_address_item=True,
...
def test_get_dns_name_bad_format_wrong_specifiers():
    ...
    with pytest.raises(ValueError) as ex:
        mount_efs.get_dns_name_and_fallback_mount_target_ip_address(config, FS_ID, DEFAULT_NFS_OPTIONS)
...
def test_get_dns_name_and_mount_target_ip_address_via_option_failure(mocker, capsys):
    ...
    dns_mock = mocker.patch('socket.gethostbyname')
    get_fallback_mount_target_ip_mock = mocker.patch('mount_efs.get_fallback_mount_target_ip_address')
    ip_address_connect_mock = mocker.patch('mount_efs.mount_target_ip_address_can_be_resolved',
                                           side_effect=mount_efs.FallbackException('Timeout'))

    with pytest.raises(SystemExit) as ex:
        mount_efs.get_dns_name_and_fallback_mount_target_ip_address(config, FS_ID, OPTIONS_WITH_IP)
